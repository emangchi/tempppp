...
then
    ...
    do
        ...
        SFTPCMD_DEBUG="lftp -d sftp://dbs_env_vib_test:1234@s-6795fc9179ba4bb29.server.transfer.ap-northeast-2.amazonaws.com \
        ...
        cls -l ${SourceFileName} | xargs > ${LastUploadFilePath}
        bye'"

        SFTPCMD="lftp sftp://dbs_env_vib_test:1234@s-6795fc9179ba4bb29.server.transfer.ap-northeast-2.amazonaws.com \
        -e 'set sftp:connect-program \"ssh -i id_rsa\";\
        set net:timeout 10;set net:max-retries 1;set net:reconnect-interval-multiplier 1;set net:reconnect-interval-base 1;\
        put -e ${SourceFilePath};\
        cls -l ${SourceFileName} | xargs > ${LastUploadFilePath}
